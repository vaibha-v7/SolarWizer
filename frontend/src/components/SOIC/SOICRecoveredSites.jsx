...
const SOICRecoveredSites = ({ alerts = [] }) => {
							...
							<div key={alert._id || `${alert.user_id}-${alert.title}`} className="rounded-xl border border-blue-100 bg-blue-50/45 p-3">
								<div className="flex items-start justify-between gap-3">
									<div className="min-w-0">
										<p className="truncate text-sm font-bold text-slate-900">{siteLabel(alert.user_name || alert.site_name, alert.user_id)}</p>
									...
									<span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-black text-emerald-700">
										{alert.status === "AUTO_RESOLVED" ? "AUTO" : "DONE"}
									</span>
								</div>
								<div className="mt-3 flex items-center justify-between gap-3 text-xs">
									<span className="font-semibold text-slate-500">{formatDate(alert.resolved_at || alert.auto_resolved_at || alert.updated_at)}</span>
